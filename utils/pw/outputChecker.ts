...
export class OutputChecker {
    ...
    static validateSoftAssertions(output: string, filePath: string): string[] {
        ...
        if (expectedIdx === -1 || checkIdx === -1) {
            throw new Error('SoftAssertions Excel must contain EXPECTED and CHECK columns');
        }
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            if (!passed) {
                errors.push(`Row ${i + 1} FAILED -> Expected: "${expected}" Check: "${checkType}"`);
            }
        }
